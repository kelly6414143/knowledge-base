...
```
你是一位資深前端工程師，請協助我為此 Vue3 專案進行以下內容。
...
找出想要目標的規則
ex:
1.先取得完整清單：用 :label="t('handle')" 搜尋，取得所有符合檔案（不設 head_limit）。
2.逐一檢查：對每個檔案的 handle column 區塊，檢查其中的 el-button 是否都有 size="small"。
...
```
你是一位資深前端工程師，請協助我為此 Vue3 專案篩選出以下內容。
...
【掃描規則】
1.先取得完整清單：用ElMessageBox搜尋，取得所有符合檔案（不設 head_limit）。
2.逐一檢查：對每個檔案的 handle column 區塊，檢查是否都有使用到 customClass: 'common-info-message-box'
3. 找出沒有使用到的頁面

【輸出需求】
條列出符合規則的頁面, 提供頁面路徑(ex: /views/...)
